...
description: |
  ...
  Each phase has a confirmation gate — never advance without user approval.
triggers:
  - user
  - model
allowed-tools:
...
6. **Confirmation gates** — Never advance to the next phase without explicit user confirmation.
...
Ask specific questions about scope, constraints, edge cases, and acceptance criteria.
Identify what is IN and what is OUT of scope.

**GATE**: Summarise your understanding and ask:
> "Entendi o seguinte: [resumo]. Posso avançar para a spec?"
...
**GATE**: Present the spec and ask:
> "Spec gerada em `sdd/specs/<feature>/spec.md`. Posso avançar para o design?"
Only proceed when user confirms.
...
**GATE**: Present the design and ask:
> "Design gerado em `sdd/specs/<feature>/design.md`. Posso avançar para o task breakdown?"
Only proceed when user confirms.
...
**GATE**: List all tasks and ask:
> "Tasks geradas em `sdd/specs/<feature>/tasks.md`. Qual task deseja executar primeiro?"
Await user choice before delegating to spdd-story.

## Phase 5: Delegate to SPDD

For each task, following dependency order, invoke the appropriate SPDD skills:

1. **spdd-story** — Break task into INVEST user stories → `sdd/specs/<feature>/stories.md`
2. **spdd-analyze** — Analyze task scope, codebase context, risks → `spdd/analysis/<task-id>-<feature>-analysis.md`
3. **spdd-canvas** — Generate REASONS Canvas → `spdd/prompts/<task-id>-<feature>-canvas.md`
4. **spdd-risk-review** — Validate risks are mitigated
5. **spdd-generate** — Implement via TDD (Red → Green → Refactor)
6. **spdd-perftest** — Run performance tests if specified
